...
### 추천 결정: 로컬 웹 어드바이저 / OBS 브라우저 오버레이

직접 TFT 클라이언트 위에 붙는 네이티브 오버레이보다, 로컬 웹 화면을 만들고 이를 OBS의 browser source로 띄우는 방식을 추천한다.

이유는 다음과 같다.

- 수업 프로젝트에서 구현 리스크가 낮다.
- Riot API key나 실시간 클라이언트 접근 문제가 있어도 데모가 가능하다.
- 게임 화면 옆에 advisor를 띄우거나, OBS에서 투명 오버레이처럼 보여줄 수 있다.
- 발표에서는 딥러닝 파이프라인 설명에 시간을 더 쓸 수 있다.

현재 구현은 Riot/League 클라이언트 lockfile을 감지하면 연결 상태를 표시한다. 하지만 추천에 필요한 세부 보드 상태는 안정적인 과제 데모를 위해 수동 선택값을 사용한다. 완전 자동 보드 인식은 화면 인식/OCR 또는 별도 game-state API 연동이 필요하므로 향후 확장 기능으로 둔다.
